    ...
    <div class="col-12">
      <div class="card">
        <DataTable paginator lazy dataKey="pk" :value="items" :rows="pagination.limit"
                   :row-hover="items.length > 0" :totalRecords="totalRecords"
                   filterDisplay="menu" :loading="loading" @sort="onSort"
                   @page="onPage" @filter="onFilter" v-model:filters="filters">
          <template #header>
          ...
          <template #empty>
            <BlankSlate icon="fa fa-tags" text="No labels found!" title="No labels!">
            </BlankSlate>
          </template>
