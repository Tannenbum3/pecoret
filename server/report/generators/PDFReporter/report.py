...
class ExecuteJavaScriptPDFGenerator(markdown.postprocessors.Postprocessor):
    def run(self, html):
        with sync_playwright() as p:
            browser = p.chromium.launch()
            context = browser.new_context()

            # Emulate network restrictions
            page = context.new_page()

            # Execute JavaScript in Playwright
            page.set_content(html)

            # Get the executed HTML content after JavaScript execution
            executed_html_content = page.content()

            # Generate PDF using WeasyPrint
            HTML(string=executed_html_content).write_pdf('output.pdf')

            browser.close()

            return executed_html_content

class ExecuteJavaScriptExtension(markdown.Extension):
    def extendMarkdown(self, md):
        md.postprocessors.register(ExecuteJavaScriptPDFGenerator(), "execute_js_pdf", 175)
...
class PDFReporter():
    ...
    NAME = "PDFReporter" # Display Name of the reporter
